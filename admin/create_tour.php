			...
			?>
				...
				<form   method="post" role="form" id="form1"   novalidate="novalidate" class="form-horizontal validate" enctype="multipart/form-data" action='<?php echo $_SERVER['PHP_SELF'] ;?>'>
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								<select style="width: 35%;"  name="supplier" id="supplier_id"  class="form-control" required>
									
								<?php
										$result_supplier = mysql_query("SELECT * FROM supplier");

										//fetch tha data from the database
										while ($row_supplier = mysql_fetch_array($result_supplier))
										{
											echo '<option value="'.$row_supplier['id'].'">'.$row_supplier['first_name'].'</option>';
