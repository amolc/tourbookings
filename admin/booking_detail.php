...
<br />		
	...
	<?php
			...
			$result = mysql_query("SELECT
									...
									INNER JOIN supplier ON supplier.id = booking.supplier_id
									INNER JOIN payment ON payment.id = booking.payment_id
									INNER JOIN traveler ON `user`.id = traveler.user_id AND tour.id = traveler.tour_id AND booking.id = traveler.booking_id
									WHERE  booking.id = '".$booking_id."'
									GROUP BY booking.id
									ORDER BY booking.id DESC 
	...
	echo '
		<h3 style="float: right;">Total Price $'.$row['total_price'].'</h3>
				...
				echo '
						...
						<table class="table table-bordered datatable" id="table-1">
								<thead>
									...
									$result2 = mysql_query("SELECT * FROM traveler where booking_id = '".$row['id']."'");
