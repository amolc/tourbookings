...
session_start();
 ...
 $tour_id = $_GET['tour_id'];
		...
		//fetch tha data from the database  WHERE p.`status` ='pending' AND tour_price.ishike ='1' 
		while ($row2 = mysql_fetch_array($result2)) 
		{ 
			$price_tour = $row2['price_customer_adult'];
		}
				...
				<?php 
					...
					// {
						...
						while ($row4 = mysql_fetch_array($sql4)) 
							...
							if($price_tour > $supplier_balance4 || $price_tour < 0)
							{
							...
							else 
							{
								...
								$total = $supplier_balance4 - $price_tour ; 
								$sql2   = "insert into supplier_balance(supplier_id,account_id,available_balance,amount_deposit,type,description,amount_withdraw,insert_date) values ('$supplier_id','$account_id','$total','0','withdraw','withdraw','$price_tour','$current_date')";
