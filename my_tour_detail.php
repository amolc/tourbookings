...
<body>
	<div id="main_container">
		...
		$query22 ="SELECT
					...
					where booking.id='".$booking_id2."' AND booking.user_id ='".$user_id."' GROUP BY tour.title
					";
					$result22 = mysql_query($query22);
		while($record2 = mysql_fetch_array($result22))
				{
				
						 $adult_result = mysql_query("select COUNT(adult_child_status) as total_adult from traveler where booking_id = '".$booking_id2."' AND user_id ='".$user_id."' AND adult_child_status='adult'");
					   while ($adult_row = mysql_fetch_array($adult_result)) {
								$total_adult = $adult_row['total_adult'];
						}
						  $child_result = mysql_query("select COUNT(adult_child_status) as total_child from traveler where booking_id = '".$booking_id2."' AND user_id ='".$user_id."' AND adult_child_status='child'");
						   while ($child_row = mysql_fetch_array($child_result)) {
							$total_child = $child_row['total_child'];
						   }
					$trip_title =  $record2['title'];
					$first_name =  $record2['first_name'];
					$last_name =  $record2['last_name'];
					$start_date2 =  $record2['start_date'];
					$end_date2 =  $record2['end_date'];
					$total_price =  $record2['total_price'];
					$country =  $record2['location_id'];
					$adult_child_status =  $record2['adult_child_status'];
					...
					?>
                        <div class="select_travel_date fl tour_detail_calendr">
							...
							<div class="Departing_tour_detail">
								...
								<input type="text" class="" value="<?php echo $start_date2 ; ?>" name="" readonly>
							</div>
                        ...
                        <div class="Adult_tour_detail">
                            <label>Adult</label>
							<input type="text" name="adult" value="<?php echo $total_adult ; ?>" readonly>
                        </div>
                        <div class="Child_tour_detail" >
                            <label>Child</label>
							<input type="text" name="child" value="<?php echo $total_child ; ?>" readonly>
                        </div>


                        <a  style="display:none" href="#" class="mrgn_top">What's this, and can I change it?</a>
                        <div style="display:none" class="update_option mrgn_top fl">
